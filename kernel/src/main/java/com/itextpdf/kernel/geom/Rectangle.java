...
public class Rectangle implements Cloneable, Serializable {
	...
	/**
	 * Gets the rectangle as it looks on the rotated page and returns the
	 * rectangle in coordinates relevant to the true page origin. This rectangle
	 * can be used to add annotations, fields, and other objects to the rotated
	 * page.
	 *
	 * @param rect
	...
	public static Rectangle getRectangleOnRotatedPage(Rectangle rect, PdfPage page) {
		...
		if (0 != rotation) {
			...
			case 1: // 90 degrees
				...
				break;
			case 2: // 180 degrees
				resultRect = new Rectangle(pageSize.getWidth() - resultRect.getRight(), pageSize.getHeight() - resultRect.getTop(), resultRect.getWidth(),
						resultRect.getHeight());
				break;
			case 3: // 270 degrees
				resultRect = new Rectangle(resultRect.getBottom(), pageSize.getHeight() - resultRect.getRight(), resultRect.getHeight(), resultRect.getWidth());
				break;
			case 4: // 0 degrees
			default:
				break;
	...
	/**
	 * Get the rectangle representation of the intersection between this
	 * rectangle and the passed rectangle
	 *
	 * @param rect
	 *            the rectangle to find the intersection with
	 * @return the intersection rectangle if the passed rectangles intersects
	 *         with this rectangle, a rectangle representing a line if the
	 *         intersection is along an edge or a rectangle representing a point
	 *         if the intersection is a single point, null otherwise
	 */
	public Rectangle getIntersection(Rectangle rect) {
		...
		// If width or height is non-negative, there is overlap and we can
		...
		if (Float.compare(width, 0) >= 0 && Float.compare(height, 0) >= 0) {
	...
	/**
	 * Check if this rectangle contains the passed rectangle. A rectangle will
	 * envelop itself, meaning that for any rectangle {@code rect} the
	 * expression {@code rect.contains(rect)} always returns true.
	 *
	 * @param rect
	 *            a rectangle which is to be checked if it is fully contained
	 *            inside this rectangle.
	 * @return true if this rectangle contains the passed rectangle, false
	 *         otherwise.
	 */
	 ...
	 *
	 * @param rect
	 *            a rectangle which is to be checked if it overlaps the passed
	 *            rectangle
	 ...
	 *            rectangle
	 * @param epsilon
	 *            if greater than zero, then this is the maximum distance that
	 *            one rectangle can go to another, but they will not overlap, if
	 *            less than zero, then this is the minimum required distance
	 *            between the rectangles so that they do not overlap
	 * @return true if there is overlap of some kind
	 */
	public boolean overlaps(final Rectangle rect, final float epsilon) {
		...
		// The first rectangle lies to the left of the second rectangle or
		// touches very slightly
		if ((this.getX() + this.getWidth()) < (rect.getX() + epsilon)) {
			return false;
		}
		// The first rectangle lies to the right of the second rectangle or
		// touches very slightly
		if ((this.getX() + epsilon) > (rect.getX() + rect.getWidth())) {
			return false;
		}

		// The first rectangle lies to the bottom of the second rectangle or
		// touches very slightly
		if ((this.getY() + this.getHeight()) < (rect.getY() + epsilon)) {
			return false;
		}
		// The first rectangle lies to the top of the second rectangle or
		// touches very slightly
	...
	/**
	 * Sets the rectangle by the coordinates, specifying its lower left and
	 * upper right points. May be used in chain. <br>
	 * <br>
	 * Note: this method will normalize coordinates, so the rectangle will have
	 * non negative width and height, and its x and y coordinates specified
	 * lower left point.
	 *
	 * @param llx
	 *            the X coordinate of lower left point
	...
	/**
	 * Increases the height of rectangle by the given value. May be used in
	 * chain.
	 *
	 * @param extra
	...
	/**
	 * Decreases the height of rectangle by the given value. May be used in
	 * chain.
	 *
	 * @param extra
	...
	/**
	 * Increases the width of rectangle by the given value. May be used in
	 * chain.
	 *
	 * @param extra
	...
	/**
	 * Decreases the width of rectangle by the given value. May be used in
	 * chain.
	 *
	 * @param extra
	 ...
	 * @param reverse
	 *            if {@code true} the rectangle will expand, otherwise it will
	 *            shrink
	...
	/**
	 * Checks if rectangle have common points with line, specified by two
	 * points.
	 *
	 * @param x1
	 ...
	 * @return {@code true} if rectangle have common points with line and
	...
	public boolean intersectsLine(float x1, float y1, float x2, float y2) {
		...
		return (rx1 <= x1 && x1 <= rx2 && ry1 <= y1 && y1 <= ry2) || (rx1 <= x2 && x2 <= rx2 && ry1 <= y2 && y2 <= ry2)
				|| linesIntersect(rx1, ry1, rx2, ry2, x1, y1, x2, y2) || linesIntersect(rx2, ry1, rx1, ry2, x1, y1, x2, y2);
	}
	...
	/**
	 * Creates a "deep copy" of this rectangle, meaning the object returned by
	 * this method will be independent of the object being cloned.
	 *
	...
	public Rectangle clone() {
		try {
			// super.clone is safe to return since all of the Rectangle's fields
			// are primitive.
	 ...
	 *
	 * @param that
	 *            the {@link Rectangle} to compare with.
	 * @return {@code true} if the difference between corresponding rectangle
	 *         values is less than deviation and {@code false} otherwise.
	 */
	...
	public static Rectangle createBoundingRectangleFromQuadPoint(PdfArray quadPoints) throws PdfException {
		...
		// QuadPoints in redact annotations have "Z" order, in spec they're
		// specified
		for (int j = 0; j < 8; j += 2) {
